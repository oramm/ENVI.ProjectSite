...
Case.prototype = {
  ...
  createDefaultTasksInDb: function(initParamObject){
    ...
    for(var k=0; k<initParamObject.defaultTaskTemplates.length; k++){
      var currentTask = this.createDefaultTask({template: initParamObject.defaultTaskTemplates[k], 
  ...
  createDefaultTask: function(initParamObject){
    if(this._type.id == initParamObject.template._caseTemplate._caseType.id){
      var currentTask = new Task({name: initParamObject.template.name,
  ...
  setDataValidation: function(){
    ...
    if (!isNaN(firstMilestoneRow) && !isNaN(lastMilestoneRow)) 
      setDataValidations([{ firstRow: firstMilestoneRow,
...
function test_deleteCase(){
  deleteCase('{"milestoneId":390,"_type":{"isDefault":"1","isUniquePerMilestone":"1","folderNumber":"01","name":"Wykonaj procedurę","id":"44","milestoneTypeId":1},"_parent":{"contractId":274,"id":390,"gdFolderId":""},"name":"wwww","typeId":"44","id":569}')
}