class Milestone {
     ...
     */
    createDefaultCasesInDb(initParamObject) {
        ...
        if (!initParamObject.defaultCaseTemplates)
            initParamObject.defaultCaseTemplates = getCaseTemplatesListPerMilestoneType({
                milestoneTypeId: this._type.id,
                isDefaultOnly: true
            }, conn);
        ...
        if (!initParamObject.defaultTaskTemplates)
            initParamObject.defaultTaskTemplates = getTaskTemplatesListPerContractType({
                contractTypeId: this._parent._type.id,
                isDefaultOnly: true
            }, conn)
        ...
        for (var j = 0; j < initParamObject.defaultCaseTemplates.length; j++) {
            var currentCaseItem = this.createDefaultCase({
                template: initParamObject.defaultCaseTemplates[j],
                foldersGdData: initParamObject.foldersGdData,
                externalConn: conn
            })
            if (currentCaseItem) {
                defaultCaseItems.push(currentCaseItem);
                var dafaultTasks = currentCaseItem.createDefaultTasksInDb({
                    defaultTaskTemplates: initParamObject.defaultTaskTemplates,
                    externalConn: conn
                });
     ...
     */
    createDefaultCase(initParamObject) {
        if (this._type.id == initParamObject.template._caseType._milestoneType.id) {
            ...
            addNewProcessInstancesForCaseInDb(currentCase, initParamObject.externalConn)
    ...
    addInScrum(format) {
        ...
        var currentTask = new Task({
            name: 'Zakończ kamień milowy lub zaktualizuj termin wykonania',
