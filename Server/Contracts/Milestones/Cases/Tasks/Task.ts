class Task {
   ...
   *
   */
  addInDbFromTemplate(parent, process, conn, isPartOfTransaction) {
  ...
  addInScrum(externalConn?, skipMakeTimesSummary?, format?) {
    var conn = (externalConn) ? externalConn : connectToSql();
    if (this.shouldBeInScrum(conn)) {
      if (!format) format = {
        fontWeight: null,
        fontStyle: null,
        fontLine: 'none'
  ...
  getParents(conn) {
    ...
    dbResults.last();
    return {
      caseName: dbResults.getString('CaseName'),
  ...
  shouldBeInScrum(externalConn): boolean {
    ...
    //zadanie nie powinno być w scrumie jeśli nie w scrumie kontraktu
    if (test) {
      var conn = (externalConn) ? externalConn : connectToSql();
      var parents = this.getParents(conn);
      //dla kontraktu 'Our' bierz dane z ourData, dla kontraktu na roboty bież dane z kolumny OurIdRelated
  ...
  initFromScrum(rowInScrumDataValues) {
    // deadline
