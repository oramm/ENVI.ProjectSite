...
function addInDb(tableName, object, externalConn, isPartOfTransaction) {
...
function editInDb(tableName, object, externalConn, isPartOfTransaction) {
  var conn = (externalConn)? externalConn : connectToSql();
  conn.setAutoCommit(false);
...
function dynamicUpdatePreparedStmtString(tableName, object){
...
function dynamicInsertPreparedStmtString(tableName, object){
...
function createPreparedStmtSql(conn, tableName, object, queryType){
  if (queryType ==='UPDATE')
