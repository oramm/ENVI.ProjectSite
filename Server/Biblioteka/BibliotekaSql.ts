...
function addInDb(
  tableName: string,
  object,
  externalConn: GoogleAppsScript.JDBC.JdbcConnection,
...
function editInDb(
  ...
  var conn: GoogleAppsScript.JDBC.JdbcConnection = externalConn
    ? externalConn
    : connectToSql();
  conn.setAutoCommit(false);
...
function deleteFromDb(
  ...
  try {
    var stmt = conn.prepareStatement(
      "DELETE FROM " + tableName + " WHERE Id =" + object.id
    );
    ...
    var end: any = new Date();
    Logger.log(
      tableName + ".deleteFromDb() :: Time elapsed: %sms for %s rows.",
      end - start,
      batch.length
    );
  } catch (e) {
...
function dynamicUpdatePreparedStmtString(tableName: string, object) {
  if (Object.getOwnPropertyNames(object)[0] !== "id")
    throw new Error(
      "Edytowany obiekt musi mieć atrybut Id jako pierwszy w konstruktorze!"
    );
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (
      (keys[i] === "id" && typeof object[keys[i]] !== "number") ||
      keys[i].indexOf("_") < 0
    ) {
...
function createPreparedStmtSql(conn, tableName, object, queryType: string) {
  ...
  else var stmtString = dynamicInsertPreparedStmtString(tableName, object);
  var stmt = conn.prepareStatement(
    stmtString,
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (keys[i].indexOf("_") < 0) {
      ...
      else stmt.setString(j++, prepareValueToPreparedStmtSql(currentValue));
    }
  }
  if (queryType === "UPDATE") stmt.setString(j++, object[keys[0]]);
  return stmt;
}
