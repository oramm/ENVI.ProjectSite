...
Gd.setFolder = function(root, name){
  name = name.trim();
  var folder;
...
Gd.createGdFolderUrl = function(gdFolderId){
  if(gdFolderId)
    return 'https://drive.google.com/drive/folders/' + gdFolderId;
}

Gd.createDocumentOpenUrl = function(gdDocumentId){
  if(gdDocumentId)
    return 'https://drive.google.com/open?id=' + gdDocumentId;
}

Gd.createDocumentEditUrl = function(gdDocumentId){
  if(gdDocumentId)
    return 'https://docs.google.com/document/d/' + gdDocumentId + '/edit';
}

Gd.createDuplicateFile = function(gdSourceFileId, destinationFolderId, fileName) {
...
Gd.canUserDeleteFile = function(fileGdId){
...
Gd.prototype = {
  ...
  createGdFolderUrl: function(gdFolderId){
    if(gdFolderId)
      return 'https://drive.google.com/drive/folders/' + gdFolderId;
  },
  ...
  searchFolderByName: function(name, rootFolder) {
    ...
    while(childFolders.hasNext()) {
      ...
      result = getSubFolders(name, child);
  ...
  makeContractFoldersDataList: function(milestone){
    ...
    var currentMilestoneFolderData = {}, 
    ...
    for (var i=0; i<caseTypes.length; i++){
      ...
      if (caseTypes[i]._milestoneType._isDefault || milestone){
        ...
        currentMilestoneFolderData = {  name: name.trim(),
        ...
        currentCaseFolderData = {  name: caseTypes[i].folderNumber + ' ' + caseTypes[i].name,
  ...
  */
  ...
  copyFolder: function(sourceFolderId, targetFolderId) {
    ...
    while(folders.hasNext()) {
      ...
      copyFolder(subFolder.getId(), targetFolder.getId());
    }  
  }
}
...
function test_moveFolder(){
  var item = JSON.parse('');
  var contract = new Contract(item)
