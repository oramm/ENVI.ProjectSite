  ...
  try {
    ...
    if (currentUserSystemRole.name == 'ENVI_EMPLOYEE' || currentUserSystemRole.name == 'ENVI_MANAGER')
      ...
      query = 'SELECT  Projects.Id, \n \t' +
        ...
        'WHERE Roles.PersonId = @x := (SELECT Persons.Id FROM Persons WHERE Persons.SystemEmail = "' + currentUser.systemEmail + '") \n' +
  ...
  try {
    ...
    while (dbResults.next()) {
      var item = new ProjectEntity({
        projectRole: dbResults.getString('ProjectRole'),
        _project: {
          id: dbResults.getLong('ProjectId')
        },
        _entity: new Entity({
