function getRisksListPerProject(projectId) {
  //projectId = 'kob.gws.01.wlasne';
  var sql = 'SELECT Risks.Id, \n \t' +
            ...
            'WHERE Risks.ProjectId=' + prepareValueToSql(projectId);
  return getRisks(sql);
}
...
function getRisks(sql) {
  ...
  try {
    ...
    while (dbResults.next()) {
      var item = new Risk ({id: dbResults.getLong('Id'),
                            ...
                            _case: {id: dbResults.getLong('CaseId'),
                                    ...
                                    _type: {name: dbResults.getString('CaseTypeName'),
                            ...
                            _parent: { id: dbResults.getLong('MilestoneId'),
                                      ...
                                      _type: { id: dbResults.getInt('MilestoneTypeId'),
                                              //folderNumber: dbResults.getString(22),
                                              name: dbResults.getString('MilestoneTypeName')
                                             },
                                      _parent: {ourIdNumberName: dbResults.getString('ContractOurId') + ' ' + dbResults.getString('ContractNumber') + ' ' + dbResults.getString('ContractName'),
...
function TEST_getRisksListPerProject(){
  getRisksListPerProject("kob.gws.01.wlasne")
}
...
function test_editRisk(itemFormClient) {
