function getMeetingsList(initParamObject, externalConn?: GoogleAppsScript.JDBC.JdbcConnection) {
...
function getMeetingsListPerContract(contractId: number) {
...
function getMeetings(sql: string, externalConn?: GoogleAppsScript.JDBC.JdbcConnection) {
    try {
        Logger.log(sql);
        var result = [];
