...
import {
    SecurityModalBodyDates,
    SecurityModalBodyDescritpion,
    SecurityModalBodyStatus,
    SecurityModalBodyValue,
} from "./Modals/SecurityModalBodiesPartial";
import {
    SecurityCashAddNewModalButton,
    SecurityEditModalButton,
    SecurityGuaranteeAddNewModalButton,
} from "./Modals/SecurityModalButtons";
import {
    securityDescriptionValidationSchema,
    securityStatusValidationSchema,
    securityValueValidationSchema,
    suecurityDatesValidationSchema,
} from "./Modals/SecurityValidationSchema";
...
export default function SecuritiesSearch({ title }: { title: string }) {
    ...
    return (
        <FilterableTable<Security>
            ...
            tableStructure={[
                { header: "Typ", renderTdBody: (security: Security) => renderType(security.isCash), colMd: 2 },
                {
                    header: "Oznaczenie",
                    renderTdBody: (security: Security) => <>{security._contract.ourId}</>,
                    colMd: 2,
                },
                { header: "Wartość", renderTdBody: (security: Security) => renderValue(security), colMd: 1 },
                { header: "Zwrócono", renderTdBody: (security: Security) => renderReturnedValue(security), colMd: 1 },
                { header: "Do zwrotu", renderTdBody: (security: Security) => renderRemainingValue(security), colMd: 1 },
                {
                    header: "70% Wygasa",
                    renderTdBody: (security: Security) => renderFirstPartExpiryDate(security),
                    colMd: 1,
                },
                {
                    header: "30% Wygasa",
                    renderTdBody: (security: Security) => renderSecondPartExpiryDate(security),
                    colMd: 1,
                },
                { header: "Uwagi", renderTdBody: (security: Security) => renderDescription(security), colMd: 2 },
            ]}
