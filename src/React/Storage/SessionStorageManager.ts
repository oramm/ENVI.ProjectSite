...
export interface StorageMetrics {
    ...
    analysis: Array<{ key: string; size: number; sizeKB: number }>;
    limitDetection: {
        detected: number;
        detectedMB: number;
...
export class SessionStorageManager {
    private static readonly WARNING_THRESHOLD = 0.8; // 80% wykorzystania
    private static _detectedLimit: number | null = null;
     ...
     */
    private static detectStorageLimit(): number {
        ...
        try {
            console.log(
                `🔍 Wykrywanie limitu storage (obecne użycie: ${((currentUsageChars * 2) / 1024 / 1024).toFixed(
                    2
                )} MB)...`
            );
            ...
            if (detectedLimitChars > 0) {
                ...
                console.log(
                    `🎯 Wykryto limit sessionStorage: ${((detectedLimitChars * 2) / 1024 / 1024).toFixed(2)} MB`
                );
                return this._detectedLimit;
            } else {
     ...
     */
    private static get STORAGE_LIMIT(): number {
     ...
     */
    static save(key: string, data: any, repositoryName: string): void {
        try {
    ...
    static getMetrics(): StorageMetrics {
        ...
        return {
            totalSize,
            totalSizeKB: (totalSize * 2) / 1024, // UTF-16: 2 bajty na znak
     ...
     */
    static resetDetectedLimit(): void {
     ...
     */
    private static analyzeStorageKeys(): Array<{ key: string; size: number; sizeKB: number }> {
        const analysis: Array<{ key: string; size: number; sizeKB: number }> = [];
     ...
     */
    private static makeStorageWarning(repositoryName: string, currentSize: number, estimatedTotalSize: number): void {
        const storageLimit = this.STORAGE_LIMIT;
     ...
     */
    private static handleStorageError(error: any, key: string, repositoryName: string, data: any): void {
        const currentStorageSize = this.calculateStorageSize();
