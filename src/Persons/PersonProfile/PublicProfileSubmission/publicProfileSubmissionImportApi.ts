import { AiProfileEducation, AiProfileExperience, AiProfileSkill } from "../../../../Typings/bussinesTypes";
import { ProfileImportApiAdapter } from "../Import/profileImportApi.types";
import { PublicProfileSubmissionApi } from "./publicProfileSubmissionApi";
 ...
 */
export function createPublicProfileSubmissionImportApi(api: PublicProfileSubmissionApi): ProfileImportApiAdapter {
    return {
        ...
        async confirmExperiences(items) {
            ...
            const newItems = items.map(({ _tempId, ...rest }: AiProfileExperience) => rest);
        ...
        async confirmEducations(items) {
            ...
            const existingPending = currentDraft.educations
                .filter(e => e.status === "PENDING")
                .map(({ id: _id, status: _s, ...rest }) => rest);
            const newItems = items.map(({ _tempId, ...rest }: AiProfileEducation) => rest);
        ...
        async confirmSkills(items) {
            ...
            const existingPending = currentDraft.skills
                ...
                .map(({ id: _id, status: _s, ...rest }) => rest);
            const newItems = items.map(({ _tempId, ...rest }: AiProfileSkill) => rest);
