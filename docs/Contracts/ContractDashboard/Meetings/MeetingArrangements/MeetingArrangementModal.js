...
class MeetingArrangementModal extends Modal {
    constructor(id, title, connectedResultsetComponent, mode) {
        ...
        this.ownerAutoCompleteTextField = new AutoCompleteTextField(this.id + '_ownerAutoCompleteTextField', 'Osoba odpowiedzialna', 'person', false, 'Wybierz imię i nazwisko');
        this.ownerAutoCompleteTextField.initialise(MainSetup.personsPerProjectRepository, '_nameSurnameEmail', undefined, this);
        this.caseCollapsibleSelect = new CollapsibleSelect(this.id + "_casesCollapsibleSelect", 'Wybierz sprawę', MeetingsSetup.milestonesRepository, (dataItem) => dataItem._type._folderNumber + ' ' + dataItem._type.name + ' | ' + dataItem.name, MeetingsSetup.casesRepository, (dataItem) => {
