...
class RiskModal extends Modal {
    constructor(id, title, connectedResultsetComponent, mode) {
        ...
        this.milestonesSelectField.initialise(RisksSetup.milestonesRepository.items.filter(item => item.contractId == RisksSetup.contractsRepository.currentItem.id), '_FolderNumber_TypeName_Name');
    ...
    onMilestoneChosen(inputValue) {
        ...
        var currentCases = RisksSetup.casesRepository.items.filter(item => item._parent.id == inputValue.id);
